...
const Home = (props) => {
  ...
  function setUpChatSocket(){
    chatSocket.current = new WebSocket('ws://'+document.location.hostname+':'+ HelperFunctions.apiPort + '/api/lobby');
  ...
  function setUpQueueSocket(matchLength){
    ...
    queueSocket.current = new WebSocket('ws://'+document.location.hostname+':'+ HelperFunctions.apiPort + '/api/queue?matchLength='+matchLength);
